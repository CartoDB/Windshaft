 ...
 * @constructor
 */
function StaticMapsController(app, previewBackend) {
    this._app = app;
...
StaticMapsController.prototype.staticMap = function(req, res, width, height, zoom /* bounds */, center) {
    ...
    step(
        ...
        function(err) {
            ...
            assert.ifError(err);
            if (center) {
                self.previewBackend.getImage(req.params, width, height, zoom, center, this);
            } else {
                self.previewBackend.getImage(req.params, width, height, zoom /* bounds */, this);
            }
        },
