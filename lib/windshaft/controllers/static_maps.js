...
function StaticMapsController(app, renderCache, mapStore, httpRendererFactory, options) {
    this._app = app;
    this._renderCache = renderCache;
    this._mapStore = mapStore;
    this._httpRendererFactory = httpRendererFactory;
...
StaticMapsController.prototype.register = function(app) {
    ...
    app.get(app.base_url_mapconfig + '/static/bbox/:token/:west,:south,:east,:north/:width/:height.:format', this.bbox.bind(this));
};
...
StaticMapsController.prototype.staticMap = function(req, res, zoom, center) {
    ...
    Step(
        ...
        function(err, mapConfig) {
            ...
            var httpLayers = mapConfig.getLayers()
                .map(function(layer, index) {
                    return {
                        layer: layer,
                        index: index,
                        isHttpLayer: layer.type === 'http'
                    };
                })
                .reduce(function(acc, current) {
                    if (current.isHttpLayer) {
                        acc.push(current);
                    }
                    return acc;
                }, []);
            ...
            var httpGetTiles = [];
            function handleHttpLayerGetTile(httpGetTile) {
                httpGetTiles.push(httpGetTile);
                if (httpLayers.length === httpGetTiles.length) {
                    var getTiles = httpGetTiles.concat(renderer.getTile.bind(renderer));
                    next(null, getTiles)
                }
            }

            httpLayers.forEach(function(layer) {
                self._httpRendererFactory.getRenderer(mapConfig, {}, format, layer.index, function(err, renderer) {
                    if (err) throw err;
                    handleHttpLayerGetTile(renderer.getTile.bind(renderer));
                })
            });
        },
        function getImage(err, getTiles) {
            if (err) throw err;
            var abaculusParams = {
                zoom: zoom,
