...
function getLayersMetadata(rendererCache, params, mapConfig, callback) {
    ...
    // WARN: this function has been behaved unexpectabilly, due to sometimes is synchronous
    // and sometimes is asynchronous. if there is no torque layers the callback is called in the same event loop cycle
    // but if there is one torque layer then the callback is called in the future (another event loop cycle). This
    // could cause that the caller hangs or, if this function fire events, bound listeners will never be called.
    // How can we fix it?. In this case, torque layers are present a considerable times. So, it makes sense
    // that the function will be allways asynchronous. For that, we can use process.nextTick to call the callback in the
