...
MapBackend.prototype.createLayergroup = function(mapConfig, params, validatorMapConfigProvider, callback) {
    ...
    if (params.filters) {
        try {
            mapConfig.setFiltersParams(JSON.parse(params.filters));
        } catch (e) {
            return callback(e);
        }
    }
...
function getLayersMetadata(rendererCache, params, mapConfig, callback) {
    ...
    mapConfig.getLayers().forEach(function(layer, layerId) {
        ...
        if (layerType === 'torque') {
            ...
            metaQueue.defer(getEmptyLayerMetadada);
        }
    });
    ...
    metaQueue.awaitAll(function (err, results) {
        ...
        mapConfig.getLayers().forEach(function(layer, layerId) {
            ...
            var widgets = mapConfig.getWidgets(layerId);

            if (_.size(widgets)) {
                metadata[layerId].widgets = widgets;
            }
        });
...
function getEmptyLayerMetadada(callback) {
