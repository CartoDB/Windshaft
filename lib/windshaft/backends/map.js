...
function MapBackend(rendererCache, mapStore) {
...
MapBackend.prototype.createLayergroup = function(requestMapConfig, params, layergroupDecorator, callback) {
    ...
    step(
        ...
        function fetchLayersMetadata(err) {
            ...
            self.getLayersMetadata(params, mapConfig, function(err, layersMetadata) {
                if (err) {
                    self._mapStore.del(mapConfig.id(), function(delErr) {
                        if (delErr) {
                            console.error("Failed to delete MapConfig '" + mapConfig.id() + " after: " + err);
...
MapBackend.prototype.getTileOrGrid = function(params, callback) {
    ...
    step(
        function() {
            ...
            self._rendererCache.getRenderer(params, this);
        },
...
MapBackend.prototype.getLayersMetadata = function(params, mapConfig, callback) {
    var self = this;
    ...
    torqueLayers.forEach(function(layerId) {
        metadataQueue.defer(function(params, token, rendererType, layerId, done) {
            self.getLayerMetadata(params, token, rendererType, layerId, done);
        }, params, mapConfig.id(), 'json.torque', layerId);
    });
...
///
MapBackend.prototype.getLayerMetadata = function(params, token, format, layerId, callback) {
    var self = this;
