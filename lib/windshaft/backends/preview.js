...
PreviewBackend.prototype.getImage = function(mapConfigProvider, format, width, height, zoom, center, callback) {
    ...
    this._getByZoomAndCenter(mapConfigProvider, format, zoom, center, options, callback);
};

PreviewBackend.prototype.getImageByZoomAndCenter = function(mapConfigProvider, format, width, height, zoom, center,
                                                            options, callback) {
    center = {
        x: center.lng,
        y: center.lat,
...
PreviewBackend.prototype.getImageByBounds = function(mapConfigProvider, format, width, height, bounds, options,
                                                     callback) {
    var zoom = boundsZoom(bounds, {
        width: width,
        height: height
    });
    var center = {
...
PreviewBackend.prototype._getByZoomAndCenter = function(mapConfigProvider, format, zoom, center, options, callback) {
    var self = this;
    ...
    step(
        ...
        function getImage(err, r) {
            ...
            var abaculusParams = {
                zoom: zoom,
                // integer between 1-4 and sets resolution (scale: 1 is 72dpi, scale: 4, is 288dpi)
