...
const RendererParams = require('../renderers/renderer_params');
...
function getSQL(attributes, pg, layer, params, testMode) {
    ...
    let quoted_att_cols = attributes.columns
        .map(n => pg.quoteIdentifier(n))
        .join(',');

    if (testMode) {
        quoted_att_cols += ',' + pg.quoteIdentifier(attributes.id);
    }

    const layerSql = SubstitutionTokens.replace(layer.options.sql, {
    ...
    let sql = `SELECT ${quoted_att_cols} FROM (${layerSql}) AS _windshaft_subquery`;
    if (!testMode) {
