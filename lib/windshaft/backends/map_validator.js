    ...
    mapConfigProvider.getMapConfig(function(err, mapConfig, params) {
        ...
        function validateMapnikTile(layerId) {
            return function(done) {
                self.tryFetchTileOrGrid(mapConfigProvider, params, token, 'png', layerId, done);
            };
        }
        ...
        if (!mapnikLayersIds.length) {
            return self._validateRemaningFormatLayers(mapConfigProvider, mapConfig, params, callback);
        }

        var validateAllMapnikTileLayers = validateMapnikTile();
        validateAllMapnikTileLayers(function (errAllLayers) {
            if (!errAllLayers) {
                return self._validateRemaningFormatLayers(mapConfigProvider, mapConfig, params, callback);
            }

            var checkFailingMapnikTileLayerFns = mapnikLayersIds.map(layerId => {
                return validateMapnikTile(layerId);
            });
...
MapValidatorBackend.prototype._validateRemaningFormatLayers = function(mapConfigProvider, mapConfig, params, callback) {
    ...
    function validateMapnikGridJson(layerId) {
        return function(done) {
            self.tryFetchTileOrGrid(mapConfigProvider, params, token, 'grid.json', layerId, done);
        };
    }

    function validateTorqueJson(layerId) {
        return function(done) {
            self.tryFetchTileOrGrid(mapConfigProvider, params, token, 'json.torque', layerId, done);
        };
    }

    function validateAttributes(layerId) {
        return function(done) {
            self.tryFetchFeatureAttributes(mapConfigProvider, params, token, layerId, done);
        };
    }
