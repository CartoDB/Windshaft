    ...
    mapConfigProvider.getMapConfig(function(err, mapConfig, params) {
        ...
        function validateMapnikTile(layerId) {
            return function(done) {
                self.tryFetchTileOrGrid(mapConfigProvider, _.clone(params), token, 'png', undefined, function (err) {
                    if (err) {
        ...
        function validateMapnikGridJson(layerId) {
            return function(done) {
                self.tryFetchTileOrGrid(mapConfigProvider, _.clone(params), token, 'grid.json', layerId,
                function (err) {
                    if (err) {
                        err.layerIndex = layerId;
                        return done(err);
        ...
        function validateTorqueJson(layerId) {
            return function(done) {
                self.tryFetchTileOrGrid(mapConfigProvider, _.clone(params), token, 'json.torque', layerId,
                function (err) {
                    if (err) {
                        err.layerIndex = layerId;
                        return done(err);
        ...
        function validateAttributes(layerId) {
            return function(done) {
                self.tryFetchFeatureAttributes(mapConfigProvider, _.clone(params), token, layerId, function (err) {
                    if (err) {
                        err.layerIndex = layerId;
                        return done(err);
                    }
        ...
        mapConfig.getLayers().forEach(function(layer, layerId) {
            ...
            if (layerType === 'mapnik') {
                ...
                if (!hasMapnikLayers) {
                    validateFnList.push(validateMapnikTile(layerId));
