...
MapValidatorBackend.prototype.validate = function(params, mapConfig, callback) {
    ...
    function validateMapnikTile() {
        return function(done) {
            self.tryFetchTileOrGrid(_.clone(params), token, 'png', undefined, done);
        };
    }

    function validateMapnikGridJson(layerId) {
        return function(done) {
            self.tryFetchTileOrGrid(_.clone(params), token, 'grid.json', layerId, done);
        };
    }

    function validateTorqueJson(layerId) {
        return function(done) {
            self.tryFetchTileOrGrid(_.clone(params), token, 'json.torque', layerId, done);
        };
    }
...
MapValidatorBackend.prototype.tryFetchTileOrGrid = function (params, token, format, layerId, callback) {
    params.token = token;
    ...
    this._mapBackend.getTileOrGrid(params, callback); // tryFetchTileOrGrid
};
