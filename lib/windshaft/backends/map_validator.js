...
MapValidatorBackend.prototype.validate = function(mapConfigProvider, callback) {
    ...
    mapConfigProvider.getMapConfig(function(err, mapConfig, params) {
        ...
        function validateAllMapnikTiles() {
            return function (done) {
                var validateAllMapnikLayers = validateMapnikTile();
                validateAllMapnikLayers(function (errAllLayers) {
                    if (!errAllLayers) {
                        return done();
                    }
                    var validateFnMapnikList = [];

                    mapConfig.getLayers().forEach(function(layer, layerId) {
                        var layerType = mapConfig.layerType(layerId);

                        if (layerType === 'mapnik') {
                            validateFnMapnikList.push(validateMapnikTile(layerId));
                        }
                    });

                    var validationMapnikQueue = queue(validateFnMapnikList.list);
                    validateFnMapnikList.forEach(function(validateFn) {
                        validationMapnikQueue.defer(validateFn);
                    });
                    validationMapnikQueue.awaitAll(function (err) {
                        return done(err || errAllLayers);
                    });
                });
            };
        }
        ...
        if (hasMapnikLayers) {
            validateFnList.push(validateAllMapnikTiles());
        }
