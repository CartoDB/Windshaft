...
MapValidatorBackend.prototype.validate = function(mapConfigProvider, callback) {
    ...
    mapConfigProvider.getMapConfig(function(err, mapConfig, params) {
        ...
        self.tryFetchTileOrGrid(mapConfigProvider, params, token, 'png', undefined, function (errAllLayers) {
            if (!errAllLayers) {
                return self._validateRemaningFormatLayers(mapConfigProvider, mapConfig, params, callback);
            }

            var checkFailingMapnikTileLayerFns = mapnikLayersIds.map(layerId => {
                return done => self.tryFetchTileOrGrid(mapConfigProvider, params, token, 'png', layerId, done);
            });
