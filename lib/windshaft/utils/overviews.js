TableNameParser = require('./table_name_parser');
...
function overviews_view_for_table(table, overviews_metadata, indent) {
    ...
    for (i = 0, len = overview_layers.length; i < len; i++) {
        ...
        ov_identifier = TableNameParser.table_identifier(ov_table);
        selects.push(indent + "SELECT * FROM " + ov_identifier + ", vovw_scale WHERE " + condition);
    }
...
function overviews_view_name(table) {
    var parsed_table = TableNameParser.parse(table);
    parsed_table.table = 'vovw_' + parsed_table.table;
...
function replace_table_in_query(sql, old_table_name, new_table_name) {
    ...
    // if new_table_name was a table it would be convenient to do this:
    //     new_table.schema = new_table.schema || old_table.schema;
    // but new_table_name may by a local name, so we'll skip it.
    ...
    sql = sql.replace(new RegExp(regexp, 'g'), new_table_ident);
    if ( old_table.schema ) {
        // We will also replace schema-less references to the table,
        // so this will not be valid if the taable name exists also
        // in other schema
        old_table.schema = new_table.schema = null;
        old_table_ident = TableNameParser.table_identifier(old_table);
        new_table_ident = TableNameParser.table_identifier(new_table);
        sql = sql.replace(new RegExp('([^\.a-z0-9_]|^)'+old_table_ident, 'g'),'$01'+new_table_ident);
    }
    return sql;
}
