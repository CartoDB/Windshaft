...
module.exports = class PostgresVectorRenderer {
    ...
    _getLayerColumns (layer) {
        return new Promise((resolve, reject) => {
            ...
            var layer_sql = this._replaceTokens(layer.options.sql, 0, 0, 0);
            var limitedQuery = `SELECT * FROM (` + layer_sql + `) __windshaft_mvt_schema LIMIT 0;`;
        ...
        return this.layers.filter(l => shouldUseLayer(l, z)).map(layer => {
            return this._getLayerColumns(layer)
                .then( layerColumns => {
                    ...
                    const finalQuery = this._vectorLayerQuery(layer.id, columns, query, { z, x, y });
                    return finalQuery;
                });
        });
    }
    ...
    _geomColumn (layer, { z, x, y }) {
        ...
        const geom_column = layer.geom_column || 'the_geom_webmercator';
        return `
        ST_AsMVTGeom(
            ${geom_column},
