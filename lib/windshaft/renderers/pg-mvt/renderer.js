...
module.exports = class PostgresVectorRenderer {
    ...
    constructor (layers, options = {}, dbParams = {}) {
        ...
        this.WebMercatorHelper = new WebMercatorHelper();
        ...
        this.subpixelBufferSize = Math.round(this.bufferSize * this.vectorExtent / this.WebMercatorHelper.tileSize);
    ...
    getTile (z, x, y, callback) {
        ...
        if (subqueries.length === 0) {
            return callback(null, new Buffer.alloc(0), { 'Content-Type': 'application/x-protobuf',
    ...
    _geomColumn (layer, z) {
        ...
        if (this.vectorExtent !== this.vectorSimplifyExtent) {
            const tol = (this.WebMercatorHelper.tileMaxGeosize / Math.pow(2, z)) / (this.vectorSimplifyExtent * 2);
    ...
    _replaceTokens (sql, { z = 0, x = 0, y = 0 }) {
        const resolution = this.WebMercatorHelper.getResolution({ z : z });
        const extent = this.WebMercatorHelper.getExtent({ x : x, y : y, z : z });
