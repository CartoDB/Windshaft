...
function setDefaultTokens (sql) {
    const extent = new CartoDBPostgisUtils().cdbXYZExtent(0, 0, 0);
...
module.exports = class PostgresVectorRenderer {
    ...
    constructor (layers, options = {}, dbParams = {}) {
        ...
        this.pgUtils = new CartoDBPostgisUtils();
        ...
        this.subpixelBufferSize = Math.round(this.bufferSize * this.vectorExtent / this.pgUtils.tileSize);
    ...
    _geomColumn (layer, z) {
        ...
        if (this.vectorExtent !== this.vectorSimplifyExtent) {
            const tol = (this.pgUtils.tileMaxGeosize / Math.pow(2, z)) / (this.vectorSimplifyExtent * 2);
