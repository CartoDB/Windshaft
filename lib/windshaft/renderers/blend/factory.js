...
BlendFactory.prototype.getRenderer = function(mapConfig, format, options, callback) {
    ...
    mapLayers.forEach(function(layer, layerIndex) {
        rendererGetTileQueue.defer(function (params, mapConfig, done) {
            ...
            var factory = self.rendererFactory.getFactory(mapConfig, layerIndex);

            if (!factory) {
                // skip/ignore layers with no factory
                return cb(null, null);
            }

            self.rendererFactory.genericMakeRenderer(
                factory, mapConfig, rendererParams, rendererParams.format, rendererParams.layer, limits, cb);
