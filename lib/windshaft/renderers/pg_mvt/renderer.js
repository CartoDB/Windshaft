    ...
    getTile: function (z, x, y, callback) {
        ...
        this.layers.forEach((layer, i) => {
            ...
            query += `coalesce((\
                ...
                (SELECT ${nonGeoColumns} ST_AsMVTGeom(the_geom_webmercator, CDB_XYZ_Extent(${x},${y},${z}), ${this.mvt_extent}, 0, true) FROM (${layer.options.sql}) AS cdbq where the_geom_webmercator && CDB_XYZ_Extent(${x},${y},${z}) ) as geom where geom is not null\
                ), '')`;
        });
        query += ') AS st_asmvt';
