...
RendererFactory.prototype.makeRendererMapnik = function(params, mapConfig, callback) {
    return genericMakeRenderer(this.mapnikRendererFactory, MapnikRenderer.adaptor, mapConfig, params, params.format, params.layer, callback);
};
...
RendererFactory.prototype.makeRendererTorque = function(params, mapConfig, callback) {
    ...
    return genericMakeRenderer(this.torqueRendererFactory, TorqueRenderer.adaptor, mapConfig, dbParams, params.format, params.layer, callback);
};

RendererFactory.prototype.makeRendererHttp = function(params, mapConfig, callback) {
    if (!validateTokenAndLayerPresence(params)) {
        return callback(createMapTokenLayerIdError('Http'));
    }

    return genericMakeRenderer(this.httpRendererFactory, HttpRenderer.adaptor, mapConfig, {}, params.format, params.layer, callback);
};

RendererFactory.prototype.makeRendererPlain = function(params, mapConfig, callback) {
    if (!validateTokenAndLayerPresence(params)) {
        return callback(createMapTokenLayerIdError('Plain'));
    }

    return genericMakeRenderer(this.plainRendererFactory, PlainRenderer.adaptor, mapConfig, {}, params.format, params.layer, callback);
};
...
RendererFactory.prototype.makeRendererBlend = function(params, mapConfig, callback) {
    ...
    return genericMakeRenderer(this.blendRendererFactory, BlendRenderer.adaptor, mapConfig, params, 'png', null, callback);
};

function genericMakeRenderer(factory, AdaptorClass, mapConfig, params, format, layer, callback) {
    step(
        function initRenderer() {
            factory.getRenderer(mapConfig, params, format, layer, this);
        },
        function makeAdaptor(err, renderer) {
            if ( err ) {
                throw err;
            }
            return new AdaptorClass(renderer, params.format);
        },
...
function createMapTokenLayerIdError(rendererType) {
    return new Error(rendererType + " renderer can only be initialized with map token and layer id");
}
