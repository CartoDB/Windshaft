...
var PgMvtRenderer = require('./pg_mvt');
...
function RendererFactory(opts) {
    ...
    if (opts.mvt.usePgSQL) {
        availableFactories.push(new PgMvtRenderer.factory(opts.torque)) // TODO abusing torque
    }
...
RendererFactory.prototype.getFactoryName = function (mapConfig, layer, format) {
    if (format === 'mvt' && this.opts.mvt.usePgSQL) {
        return 'pg_mvt';
    }
...
function isMapnikFactory(mapConfig, layer, format) {
    ...
    if (layersFilter.isFilter(layer) && isEveryMapnik(mapConfig, layer) && format !== 'grid.json') {
        return true;
    }
