...
MapnikFactory.prototype.getRenderer = function(mapConfig, format, options, callback) {
    ...
    params = _.defaults(params, this.mapConfigToMMLBuilderConfig(mapConfig, this._mapnik_opts.overviewsHandler));
...
MapnikFactory.prototype.mapConfigToMMLBuilderConfig = function(mapConfig, overviewsHandler) {
    ...
    for ( var i=0; i<cfg.layers.length; ++i ) {
        ...
        var query = lyropt.sql, cartocss = lyropt.cartocss;
        if ( overviewsHandler ) {
            query = overviewsHandler.query(lyropt.sql, overviews_metadata);
            cartocss = overviewsHandler.style(lyropt.cartocss, lyropt.cartocss_version, overviews_metadata);
        }
