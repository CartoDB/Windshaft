...
function overviewsMetadata(mapConfig, params, callback) {
    // TODO: use the affected tables info from W-cartodb => CDB_Overviews
    var affected_tables = [ 'test_table' ];

    // Obtain overviews metadata (for testing we're using only one table, there
    // could be more)
    ...
    console.log('mapnik layers queries: %j', mapnikLayers.map(function(layer) { return layer.options.sql; }));

    var overviews_md = {};

    // Should be SELECT * FROM CDB_Overviews(affected_tables[0])
    pg.query("SELECT * FROM CDB_Overviews('" + affected_tables[0] + "');", function(err, result) {
        if (err) {
            return callback(err);
        }
        overviews_md[affected_tables[0]] = {};
        for ( var i=0; i<result.rows.length; ++i ) {
          var row = result.rows[i];
          overviews_md[affected_tables[0]][row.z] = row.overview_table;
        };
