...
var cartoPSQLFactory = require('./carto_psql_factory');
...
function MapnikFactory(options) {
    ...
    // FIXME: this config should come from the client (e.g: CartoDB-Windshaft)
    this._options.geojson = {
        dbPoolParams: {
...
MapnikFactory.prototype.getGeoJsonRenderer = function(mapConfig, format, options, callback) {
    ...
    var dbPoolParams = this._options.geojson.dbPoolParams;
    var cartoPSQL = cartoPSQLFactory(dbParams, dbPoolParams);
    ...
    if (typeof layerFilter === 'number') {
        ...
        filteredLayers = filteredLayers.sort(function(a, b) {
            return a - b;
        });
    ...
    callback(null, new GeojsonRenderer(cartoPSQL, layers));
};
