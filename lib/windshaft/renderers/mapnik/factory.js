...
MapnikFactory.prototype.getGeoJsonRenderer = function(mapConfig, format, options, callback) {
    ...
    var rendererOptions = this._mapnik_opts.geojson || {};
    var bufferSize = this.getBufferSize(mapConfig, format, rendererOptions);
    rendererOptions.bufferSize = bufferSize;
    var layers = [];
    ...
    if (typeof layerFilter === 'number') {
        ...
        try {
            filteredLayers = layersFilter(mapConfig, layerFilter);
        } catch (err) {
            return callback(err);
        }
    ...
    callback(null, new GeojsonRenderer(cartoPSQL, layers, rendererOptions));
};
...
MapnikFactory.prototype.mapConfigToMMLBuilderConfig = function(mapConfig, queryRewriter, layerFilter) {
    ...
    filteredLayerIndexes.filter(layerIndex => {
