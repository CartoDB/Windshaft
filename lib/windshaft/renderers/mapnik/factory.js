...
MapnikFactory.prototype.getRenderer = function (mapConfig, format, options, callback) {
    ...
    if (options.params['cache-features'] === undefined) {
        options.params['cache-features'] = this._mapnik_opts['cache-features'];
    }
...
MapnikFactory.prototype.mapConfigToMMLBuilderConfig = function(mapConfig, queryRewriter, rendererOptions) {
    ...
    filteredLayerIndexes.reduce(function (options, layerIndex) {
        ...
        if ( lyropt.hasOwnProperty('cache-features') ) {
            options['cache-features'].push(lyropt['cache-features']);
        } else {
            options['cache-features'].push(rendererOptions.params['cache-features']);
        }
