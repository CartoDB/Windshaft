...
var Overviews = require('../../utils/overviews');
...
MapnikFactory.prototype.getRenderer = function(mapConfig, format, options, callback) {
    ...
    step(
        function prepareParams() {
            self.mapConfigToMMLBuilderConfig(mapConfig, params, self, this);
        },
        function initBuilder(err, _params) {
            assert.ifError(err);

            params = _.defaults(params, _params);
...
MapnikFactory.prototype.mapConfigToMMLBuilderConfig = function(mapConfig, params, factory, callback) {
    var cfg = mapConfig.obj();
    var sql_options = []; // layer options for building sql queries
    var style = [];
    ...
    for ( var i=0; i<sql_options.length; ++i ) {
      q.defer(function(done){
        var lyropt = sql_options[i];

        overviewsMetadata(mapConfig, params, function(err, overviews_metadata) {
          if (err) {
            done(err);
          } else {
            var query = Overviews.query(lyropt.sql, overviews_metadata);
            var style = Overviews.style(lyropt.cartocss, lyropt.cartocss_version, overviews_metadata);

            var queryOptions = prepareQuery(query, lyropt.geom_column, lyropt.geom_type, factory._mapnik_opts);
            var geo_col_info = {
                type: queryOptions.geomColumnType, // grainstore allows undefined here
                name: queryOptions.geomColumnName
            };
            done(null, [queryOptions.sql, geo_col_info, style]);
            return done(null);
          };
        });
      });
    ...
    q.awaitAll(function(err, results){
      ...
      for ( var i=0; i<results.length; ++i ) {
        sql.push(results[i][0]);
        geom_columns.push(results[i][1]);
        style.push(results[i][2][0]);
        style_version.push(results[i][2][1]);
      }
...
function overviewsMetadata(mapConfig, params, callback) {
    // TODO: get affected tables by all mapConfig queries
    // var mapnikLayers = mapConfig.getLayers().filter(function(layer, layerIndex) {
    //     return mapConfig.getType(layerIndex) === 'mapnik';
    // });
    // for ( var i=0; i<mapnikLayers.length; ++i ) {
    //   // ... obtain tables affected by mapnikLayers[i].options.sql ...
    // };
    var affected_tables = [ 'cities_subset' ];
    ...
    // TODO: can simplify this by having CDB_Overviews accept multiple table
    // names and return a TABLE(table, z, overview_table)
    for ( var i=0; i < affected_tables.length; ++i ) {
      q.defer(function(done){
        var affected_table = affected_tables[i];
        pg.query("SELECT * FROM CDB_Overviews('" + affected_table + "');", function(err, result) {
            if (err) {
              done(err);
            } else {
              overviews_md[affected_table] = {};
              for ( var i=0; i<result.rows.length; ++i ) {
                var row = result.rows[i];
                overviews_md[affected_table][row.z] = row.overview_table;
              };
              done(err);
            }
        }, true);
    ...
    q.awaitAll(function(err, results){
      if (err) {
          ...
          return callback(null, overviews_md);
      }
    });
}
