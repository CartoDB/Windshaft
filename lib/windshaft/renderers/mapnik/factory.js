...
var QueryRewriter = require('../../utils/query_rewriter');
...
function MapnikFactory(options) {
    ...
    this._mapnik_opts = _.defaults(options.mapnik || {}, {
        ...
        // A query-rewriter can be passed to preprocess SQL queries and
        // CartoCSS before it is passed to Mapnik.
        // The query rewriter should contain to functions:
        // - `query(sql, data)` to transform queries using the optional data provided
        // - `style(cartocss, cartocssversion, data)` to transform cartocss
        // The data passed to these functions can be provided for eaach layer
        // through a `query_rewrite_data` entry in the layer options.
        // By default a dummy query rewriter which doesn't alter queries
        // nor style is used.
...
MapnikFactory.prototype.getRenderer = function(mapConfig, format, options, callback) {
    ...
    params = _.defaults(params, this.mapConfigToMMLBuilderConfig(mapConfig, this._mapnik_opts.queryRewriter));
...
MapnikFactory.prototype.mapConfigToMMLBuilderConfig = function(mapConfig, queryRewriter) {
    ...
    for ( var i=0; i<cfg.layers.length; ++i ) {
        ...
        var cartocss = queryRewriter.style(lyropt.cartocss, lyropt.cartocss_version, lyropt.query_rewrite_data);

        style.push(cartocss);
        style_version.push(lyropt.cartocss_version);

        var queryOptions = prepareQuery(query, lyropt.geom_column, lyropt.geom_type, this._mapnik_opts);
