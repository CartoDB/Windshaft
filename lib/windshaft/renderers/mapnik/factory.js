...
var TurboCartocssParser = require('../../style/turbo-cartocss-parser');
...
MapnikFactory.prototype.getMapnikRenderer = function(mapConfig, format, options, callback) {
    ...
    step(
        function parseCartoCss () {
            var next = this;

            var requestDbParams = RendererParams.dbParamsFromReqParams(options.params);
            var datasource = mapConfig.getLayerDatasource(options.layer);
            var dbParams = _.extend({}, requestDbParams, datasource);
            var psql = new CartoPSQL(dbParams);

            var turboCartoCssParser = new TurboCartocssParser(psql);
            turboCartoCssParser.process(params.style, params.originalSql,  next);
        },
        function initBuilder(err, cartocss) {
            // Ignore turbo-cartocss errors and continue to carto parser
            if (!err) {
                params.style = cartocss;
            }
