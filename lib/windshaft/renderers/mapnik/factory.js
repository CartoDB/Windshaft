...
function prepareQuery(userSql, geomColumnName, geomColumnType) {
    ...
    if (geomColumnType === COLUMN_TYPE_GEOMETRY) {
        sql = [
            'WITH inner_cdbq as (' + userSql + ')',
            'SELECT *, ' + geomColumnName + ' ' + MAPNIK_COLUMN_NAME,
            'FROM inner_cdbq'
        ];
        ...
        if (!!global.environment.renderer.snapToGrid && !!global.environment.renderer.clipByBox2d) {
            sql = [
                ...
                'CASE WHEN CDB_ScaleDenominatorToZoom(!scale_denominator!) > 8',
                    'THEN ST_SnapToGrid(ST_ClipByBox2D(' + geomColumnName + ', !bbox!), greatest(!pixel_width!,!pixel_height!)/20.0)',
            ...
            sql = [
                'WITH inner_cdbq as (' + userSql + ')',
                'SELECT *, ST_SnapToGrid(' + geomColumnName + ', greatest(!pixel_width!,!pixel_height!)/20.0) ' + MAPNIK_COLUMN_NAME,
                'FROM inner_cdbq'
            ];
        }

        // Override geom column name as we modified it in the queries for column type = geometry
