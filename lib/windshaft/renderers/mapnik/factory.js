...
MapnikFactory.prototype.getRenderer = function (mapConfig, format, options, callback) {
    ...
    if (mapConfig.isVectorOnlyLayergroup() && format !== 'mvt') {
        const error = new Error('Invalid format: only mvt format is available for layers without CartoCSS defined');
...
function validateLayer(mapConfig, layerIndex) {
    ...
    if (!mapConfig.isVectorOnlyLayergroup()) {
