...
MapnikFactory.prototype.getMapnikRenderer = function(mapConfig, format, options, callback) {
    ...
    var cartoCSSList = mapConfig.getCartoCSSFromLayers();
    var bufferSizeFromCartoCSS = cartocssUtils.getMaxBufferSizeFromCartoCSS(cartoCSSList);
    ...
    step(
        ...
        function loadMapnik(err, xml) {
            ...
            var bufferSize = self._mapnik_opts.bufferSize;
            if (bufferSize < bufferSizeFromCartoCSS) {
                bufferSize = bufferSizeFromCartoCSS;
            }
