...
MapnikFactory.prototype.getRenderer = function (mapConfig, format, options, callback) {
    ...
    let grainstoreOptions = this._options.grainstore;
    if (isMvt) {
        try {
            grainstoreOptions = getMVTOptions(mapConfig, this._options.grainstore);
        } catch (error) {
            return callback(error);
        }
    }

    this._mmlStore = new grainstore.MMLStore(grainstoreOptions);
    let mmlBuilder;
    try {
        mmlBuilder = this._mmlStore.mml_builder(params, mmlBuilderOptions);
    } catch (error) {
        return callback(error);
    }
    ...
    mmlBuilder.toXML((err, xml) => {
        ...
        const query = {
            metatile: this.getMetatile(format),
            metatileCache: this._mapnik_opts.metatileCache,
