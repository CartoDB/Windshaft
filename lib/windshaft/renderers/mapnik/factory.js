...
MapnikFactory.prototype.getBufferSizeFromOptions = function(format) {
    return Number.isFinite(this._mapnik_opts.formatBufferSize[format]) ? 
        this._mapnik_opts.formatBufferSize[format] :
        this._mapnik_opts.bufferSize;
};

MapnikFactory.prototype.getBufferSize = function(mapConfig, format) {
    var bufferSize = this.getBufferSizeFromOptions(format);

    if (Number.isFinite(mapConfig.getBufferSize(format))) {
        bufferSize = mapConfig.getBufferSize(format)
    }
