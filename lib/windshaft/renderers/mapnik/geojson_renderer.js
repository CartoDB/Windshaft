...
function GeojsonRenderer(cartoPSQL, mapConfig) {
...
GeojsonRenderer.prototype.getTile = function(z, x, y, callback) {
    ...
    var rendererGetTileQueue = queue(this.mapConfig.getLayers().length);
    
    this.mapConfig.getLayers().forEach(function(layer, layerId) {
        var layerType = self.mapConfig.layerType(layerId);
