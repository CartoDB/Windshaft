...
GeojsonRenderer.prototype._getTileData = function(cartoPSQL, coord, zoom, layer, layerMetadata, callback) {
    ...
    var extent = cartoDBPostgisUtils.cdbXYZExtent(coord.x, coord.y, zoom);
    var geomColumnSrid = layer.options.srid || 3857;
    ...
    var query = this._composeGeojsonQuery({
        ...
        bufferSize: getBufferSize(layerMetadata, this.bufferSize),
        extent: extent,
...
function getBufferSize(layerMetadata, defaultValue) {
    layerMetadata = layerMetadata || {};
    var bufferSize = Number.isFinite(layerMetadata['buffer-size']) ? layerMetadata['buffer-size'] : defaultValue;
    return Math.max(Math.min(256, bufferSize), 0);
}
