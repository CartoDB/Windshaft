...
TorqueFactory.prototype = {
    ...
    getRenderer: function(mapConfig, format, options, callback) {
        ...
        fetchMapConfigAttributes(pgSQL, layerObj, function (err, params) {
            ...
            parseCartoCSS(pgSQL, layerObj.options.cartocss, layerObj.options.sql, function (err, cartocssParsed) {
                if (err) {
                    return callback(err);
                }

                layerObj.options.cartocss = cartocssParsed;
...
function parseCartoCSS(pg, cartocss, query, callback) {
    var turboCartoCssParser = new TurboCartocssParser(pg);

    turboCartoCssParser.process(cartocss, query, function (err, cartocssParsed) {
        if (err) {
            return callback(err);
        }
