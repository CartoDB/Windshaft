...
function fetchLayerAttributes(sql, layer_sql, attrs, callback) {
    ...
    var query = `select * from (${layer_sql}) __torque_wrap_sql limit 0`;
    sql(query, function (err, data) {
        ...
        if (!data.fields.hasOwnProperty(attrs.column)) {
            ...
            const columnError = new Error(`TorqueRenderer: Layer query did not return the requested time-attribute column '${attrs.column}'`);
            return callback(columnError);
        }

        is_time = data.fields[attrs.column].type === 'date';
        let max_col = `max(${attrs.column})`;
        let min_col = `min(${attrs.column})`;
        if (is_time) {
        ...
        `;
        sql(query, function generateMetadata(err, data) {
