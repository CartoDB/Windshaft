...
TorqueFactory.prototype = {
    ...
    getRenderer: function(mapConfig, format, options, callback) {
        ...
        fetchMapConfigAttributes(pgSQL, layerObj, function (err, params) {
            ...
            var layerOptions = {
                cartocss: layerObj.options.cartocss,
                sql: layerObj.options.sql
            };
            parseCartoCSS(self.sqlClass, dbParams, layerOptions, function (err, css) {
                // Ignore turbo-cartocss errors and continue to carto parser
                if (!err) {
                    layerObj.options.cartocss = css;
                }
...
function parseCartoCSS(SQLClass, dbParams, layerOptions, callback) {
    var pg = new SQLClass(dbParams);
    var turboCartoCssParser = new TurboCartocssParser(pg);

    turboCartoCssParser.process(layerOptions.cartocss, layerOptions.sql, function (err, cartocssParsed) {
        if (err) {
            return callback(err);
        }

        if (Array.isArray(layerOptions.cartocss)) {
            return callback(null, cartocssParsed);
        }

        callback(null, cartocssParsed[0]);
    });
}
