...
TorqueFactory.prototype = {
    ...
    getRenderer: function(mapConfig, format, options, callback) {
        ...
        fetchMapConfigAttributes(pgSQL, mapConfig, layer, function (err, params) {
            if (err) {
                callback(err);
                return;
            }
...
function fetchMapConfigAttributes(sql, mapConfig, layer_id, callback) {


  // only one layer supported
  var layer = mapConfig.getLayer(layer_id);

  if (!layer) {
    return callback(new Error("mapconfig does not contain any torque layer"));
  }
