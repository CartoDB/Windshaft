...
function PngRenderer(layer, sql, attrs) {
    ...
    this.provider = new torque.providers.windshaft(Object.assign(
        {
            no_fetch_map: true,
...
PngRenderer.prototype.getTile = function(z, x, y, callback) {
    var self = this;
    var attrs = Object.assign({stepSelect: this.step, stepOffset: this.stepOffset}, this.attrs);
    this.getTileData(this.sql, {x: x, y: y}, z, this.layer.options.sql, attrs, function(err, rows, headers, stats) {

        var timer = new Timer();
        timer.start('render');
        var canvas = createCanvas(self.tile_size, self.tile_size);
        ...
        try {
            pointRenderer.renderTile(self.provider.proccessTile(rows, {x: x, y: y}, z), self.step, function(err) {
                ...
                canvas.toBuffer(function (err, buf) {
                    timer.end('encode');
                    callback(err, buf, {'Content-Type': 'image/png'}, Object.assign(timer.getTimes(), stats));
                });
            });
        } catch (err) {
            callback(err, null, {});
