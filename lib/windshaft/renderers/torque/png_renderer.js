...
function PngRenderer(layer, sql, attrs) {
    ...
    this.provider = new torque.providers.windshaft(_.extend({ doNotFetchMap: true }, cartoCssOptions));
...
PngRenderer.prototype.getTile = function(z, x, y, callback) {
    ...
    this.getTileData(this.sql, {x: x, y: y}, z, this.layer.options.sql, this.attrs, function(err, rows) {
        ...
        var step = 0; // TODO dummy step, TBD how to pick this value from configuration/options/request

        try {
            pointRenderer.renderTile(self.provider.proccessTile(rows, {x: x, y: y}, z), step);
        } catch (err) {
            callback(err, null, {});
        }
