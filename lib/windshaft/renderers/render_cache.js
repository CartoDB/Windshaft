...
module.exports = function(options, mml_store, map_store, mapnik_opts, httpRendererFactory) {
        ...
        if (this.shouldRecreateRenderer(cache_entry, cache_buster)) {
            ...
            Step(
              ...
              function makeRenderer (err, mapConfig) {
                if ( err ) { callback(err); return; }
                that.makeRenderer(req, mapConfig, function(err, renderer) {
                  if (!err && req.profiler) {
    ...
    me.makeRendererMapnik = function(req, mapConfig, callback){
    ...
    // controls the instantiation of mapnik renderer objects from mapnik XML
    me.makeRendererTorque = function(req, mapConfig, callback){
        var that = this;
        var params = _.extend({}, req.params);
    ...
    me.makeRendererHttp = function(req, mapConfig, callback) {
        var params = _.extend({}, req.params);
        if ( ! params.token || ! params.hasOwnProperty('layer') ) {
            callback(new Error("Torque renderer can only be initialized with map token and layer id"));
            return;
    ...
    me.makeRenderer = function(req, mapConfig, callback) {
      var params = req.params;
      if ( params.layer === 'all' ) {
        callback(Error("Not implemented"))
      }
      else if ( params.format.match(/^(png|grid\.json)$/) ) {
        me.makeRendererMapnik(req, mapConfig, callback);
      }
      else if ( params.format.match(/torque/) ) {
        me.makeRendererTorque(req, mapConfig, callback);
      }
      else if ( params.format.match(/http/) ) {
        me.makeRendererHttp(req, mapConfig, callback);
      }
      else {
