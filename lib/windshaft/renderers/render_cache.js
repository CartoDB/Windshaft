...
module.exports = function(options, mml_store, map_store, mapnik_opts, rendererFactory) {
    ...
    me.dumpStats = function() {
      ...
      var nextGc = timeout - (now - me.lastCacheClearRun);
      console.log("RenderCache items " + itemkeys.length + " timeout " + timeout + " next gc in " + nextGc);
      _.each(itemkeys, function(key){
        var item = renderers[key];
        var ttl = timeout - item.timeSinceLastAccess(now);
        console.log(" ItemKey " + key + " ttl " + ttl + " cb " + item.cache_buster );
      });
    };
    ...
    me.getRenderer = function(req, callback) {
        ...
        if (this.shouldRecreateRenderer(cache_entry, cache_buster)) {
            ...
            step(
              function invokeHook () {
                if ( ! params.processRendererCache ) {
                    return true;
                }
