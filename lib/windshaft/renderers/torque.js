...
Renderer.prototype = {
  ...
  getTileData: function(sql, coord, zoom, layer_sql, attrs, callback) {
    ...
    var tile_sql = "" +
        "WITH par AS (" +
            "WITH innerpar AS (" +
                "SELECT ST_MakeEnvelope({xmin}, {ymin}, {xmax}, {ymax}, {srid}) as ext" +
            ") " +
            "SELECT " +
                ...
                "st_xmax(innerpar.ext) as xmax, " +
                "st_ymin(innerpar.ext) as ymin, " +
                "st_ymax(innerpar.ext) as ymax " +
            "FROM innerpar" +
        ") " +
        "SELECT xx x__uint8, " +
             "yy y__uint8, " +
        ...
        "FROM ( " +
        "select " +
           "GREATEST(0, LEAST(round((p.xmax - p.xmin)*resinv) - 1, round((st_x(i.{gcol}) - p.xmin)*resinv))) as xx, " +
           "GREATEST(0, LEAST(round((p.ymax - p.ymin)*resinv) - 1, round((st_y(i.{gcol}) - p.ymin)*resinv))) as yy " +
           ", {countby} c " +
