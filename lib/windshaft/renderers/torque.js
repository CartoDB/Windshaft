...
function PSQLAdaptor(psqlClass) {
  ...
  ctor.prototype.query = function(sql, callback) {
    this._psql.query(sql, this._handleResult.bind(this, callback));
  };
...
Renderer.prototype = {
  ...
  getTile: function(x, y, z, callback) {
    _getTileData(this.sql, {x: x, y: y}, z, this.layer_sql, this.attrs, callback);
  },
  ...
  getMetadata: function() {
    ...
    return {
      ...
      columnType: a.is_time ? 'date': 'number'
    };
  }
};
...
TorqueFactory.prototype = {
  ...
  getRenderer: function(mapConfig, dbParams, format, layer, callback) {
    ...
    if (mapConfig.layers[layer] === undefined) {
    ...
    fetchMapConfigAttributes(pgSQL, mapConfig, dbParams, layer, function(err, params){
      ...
      callback(null, new Renderer(mapConfig.layers[layer], pgSQL, params, layer));
    });
  }
...
function _getTileData(sql, coord, zoom, layer_sql, attrs, callback) {
    ...
    //TODO: move to options, allow to write in MapConfig
    var tile_size = 256;
    var tile_max_geosize = 40075017; // earth circumference in webmercator 3857
    var geom_column = 'the_geom_webmercator'; 
    var geom_column_srid = 3857;
    ...
    var tile_sql = "" +
      ...
      geom_column_srid + ") as ext )," +
      "cte AS ( "+
      "  SELECT ST_SnapToGrid(i." + geom_column + ", p.res) g" +
      ", {countby} c" +
      ", floor(({column_conv} - {start})/{step}) d" +
      "  FROM ({_sql}) i, par p " +
      "  WHERE i." + geom_column + " && p.ext " +
      "  GROUP BY g, d" +
      ") " +
      "" +
    ...
    sql(query, function (err, data) {
      if (err) callback(new Error("Error running query " + query + ": " + err));
