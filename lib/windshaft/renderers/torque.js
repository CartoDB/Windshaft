...
Renderer.prototype = {
  ...
  getTileData: function(sql, coord, zoom, layer_sql, attrs, callback) {
    ...
    function CDB_XYZ_Extent_Without_Upper_Border(x, y, z, resolution) {
        ...
        var xmax = -origin_shift + (x+1)*tile_geo_size - pixres*resolution*0.5;

        // tile coordinate system is y-reversed so ymin is the top of the tile
        var ymin = origin_shift - y*tile_geo_size - pixres*resolution*0.5;
    ...
    var tile_sql = "" +
      ...
      "SELECT xx x__uint8, " +
       ...
       "FROM ( " +
       "select " +
           "round((st_x(i.{gcol}) - st_xmin(p.ext))*resinv) as xx, " +
           "round((st_y(i.{gcol}) - st_ymin(p.ext))*resinv) as yy " +
           ", {countby} c " +
    ...
    var query = format(tile_sql, attrs, {
      ...
      gcol: geom_column
    }, CDB_XYZ_Extent_Without_Upper_Border(coord.x, coord.y, zoom, attrs.resolution));

    console.log('rochoa ->', query);
