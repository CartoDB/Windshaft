...
//
function attrsFromCartoCSS(cartocss) {
  var attrs_keys = {
...
// 
function _checkLayerAttributes(layerConfig, callback) {
  var cartoCSS = layerConfig.options.cartocss;
  if (!cartoCSS)
    throw Error("cartocss can't be undefined");
  var attrs = attrsFromCartoCSS(cartoCSS);
  var checks = ['steps', 'resolution', 'column'];
  _(checks).each(function(c) {
    if (!attrs[c]) {
      throw Error( c + "must be set");
    }
  });
  return attrs;
}

function Renderer(mapConfig, dbParams, attrs) {
...
Renderer.prototype = {
  // coord: { x, y, zoom } to render
  // callback: will be called when done using nodejs protocol (err, data)
  getTile: function(x, y, z, callback) {
    _getTileData(sql(this.dbParams), {x: x, y: y}, z, this.attrs, this.layer_sql, callback);
  }
};
...
module.exports = {
  ...
  /// @param callback funcition called when method finishes. Takes (err, data), when error is not null an error occurred
  getRenderer: function(mapConfig, dbParams, format, layer, callback) { 
    ...
    if(!_.contains(this.supported_formats, format)) {
      callback("format not supported");
      return;
    }
    fetchMapConfigAttributes(mapConfig, dbParams, layer, function(err, params){
      if (err) {
        callback(err);
        return;
      }
      callback(null, new Renderer(mapConfig, dbParams, params, layer));
    });
  }
...
function fetchMapConfigAttributes(mapConfig, dbParams, layer_id, callback) {
  ...
  // check attrs
  var attrs = _checkLayerAttributes(layer);

  // fetch layer attributes to check the query and so on is ok
  var layer_sql = layer.options.sql;
  fetchLayerAttributes(sql(dbParams), layer_sql, attrs, callback);
}

function sql(dbParams) {
