...
CacheEntry.prototype._destroy = function() {
    ...
    if (!this.ready) {
        ...
        console.log(`cache_entry ${this.key} isn't ready yet, scheduling destruction on 'ready' event`); // jshint ignore:line
        this.on('ready', this._destroy.bind(this));
        return;
    }

    if (!this.renderer) {
        console.log(`Cache entry ${this.key} is ready but has no renderer, so nothing to do`); // jshint ignore:line
        return;
    }
    ...
    this.renderer.close(() => {
        console.log(`Mapnik Renderer ${this.key} closed due to its cache entry was released and it doesn\'t have references`); // jshint ignore:line
    });
};
