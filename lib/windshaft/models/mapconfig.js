...
MapConfig.prototype.setFiltersParams = function(filters, params, callback) {
    ...
    if (dataviewsFilters && !Array.isArray(filters.layers)) {
        filters.layers = [];
        this.getLayers().forEach(function(layer, layerIndex) {
            Object.keys(self._filters[layerIndex]).forEach(function(filterName) {
                if (dataviewsFilters.hasOwnProperty(filterName)) {
                    filters.layers[layerIndex] = filters.layers[layerIndex] || {};
                    filters.layers[layerIndex][filterName] = dataviewsFilters[filterName];
                }
            });
        });
    }
