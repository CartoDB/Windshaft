...
MapConfig.prototype.getBufferSize = function (format) {
    if (this._cfg.buffersize && Number.isFinite(Number(this._cfg.buffersize[format]))) {
        return Number(this._cfg.buffersize[format]);
    }

    if (Number.isFinite(Number(this._cfg.buffersize))) {
        return Number(this._cfg.buffersize);
    }
