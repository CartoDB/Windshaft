...
MapConfig.prototype.hasIncompatibleLayers = function () {
    return !this.isVectorOnlyLayergroup() && this.hasVectorLayer();
};

MapConfig.prototype.isVectorOnlyLayergroup = function () {
    const layers = this._cfg.layers;
    let isVectorOnlyLayergroup = false;

    if (!layers.length) {
        return isVectorOnlyLayergroup;
    }

    isVectorOnlyLayergroup = true;

    for (const layer of layers) {
        if (!this.isVectorLayer(layer)) {
            isVectorOnlyLayergroup = false;
            break;
        }
    }
...
MapConfig.prototype.isVectorLayer = function (layer) {
