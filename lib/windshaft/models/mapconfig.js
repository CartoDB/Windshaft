 ...
 * @returns {*}
 */
MapConfig.prototype.getList = function(layerIndex, listName) {
    return this._getWidget(layerIndex, listName, widget.List);
};

MapConfig.prototype.getHistogram = function(layerIndex, histogramName) {
    return this._getWidget(layerIndex, histogramName, widget.Histogram);
};

MapConfig.prototype._getWidget = function(layerIndex, name, ResourceClass) {
    var layerWidgets = this._widgets[layerIndex];
    if (!layerWidgets) {
        throw new Error('Layer ' + layerIndex + ' not found');
    }

    var widget = layerWidgets[name];

    if (!widget) {
        throw new Error("Widget '" + name + "' not found at layer " + layerIndex);
    }

    if (ResourceClass.type !== widget.type) {
        throw new Error("Incompatible widget type '" + ResourceClass.type + "', found " + widget.type);
    }

    return new ResourceClass(widget.options || {}, this.getLayer(layerIndex).options || {});
};
