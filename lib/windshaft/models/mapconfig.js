...
function MapConfig(config, datasource) {
    ...
    if ( layer.options.attributes ) {
        if ( layer.options.sql.match(mapnikTokens_RE) ) {
            throw new Error(
                "Attribute service cannot be activated on layer " + i + ": using dynamic sql (mapnik tokens)"
            );
        }
    }
    });
    ...
    */
    ...
    this._widgets = this.getLayers().map(function(layer) {
        return layer.options.widgets || {};
    });

    this._filters = this.getLayers().map(function(layer) {
        var filters = {};
...
MapConfig.prototype.getWidget = function(layerIndex, name, bbox) {
    var layerWidgets = this._widgets[layerIndex];
    if (!layerWidgets) {
        throw new Error('Layer ' + layerIndex + ' not found');
    }

    var widgetDefinition = layerWidgets[name];

    if (!widgetDefinition) {
        throw new Error("Widget '" + name + "' not found at layer " + layerIndex);
    }

    return widgetFactory.getWidget(widgetDefinition, this.getLayer(layerIndex).options || {}, bbox);
};
...
MapConfig.prototype.applyFilters = function(filters) {
    var deepCopyMapConfig = JSON.parse(JSON.stringify(this._cfg));
    var mapConfig = MapConfig.create(deepCopyMapConfig, this._datasource.clone());
    if (Array.isArray(filters.layers)) {
        filters.layers.forEach(function(layerFilters, layerIndex) {
            Object.keys(layerFilters).forEach(function(filterName) {
                var filterDefinition = mapConfig._filters[layerIndex][filterName];
                if (filterDefinition) {
                    var filter = filterFactory.getFilter(
                        mapConfig.getLayer(layerIndex), filterDefinition, layerFilters[filterName]
                    );
                    mapConfig._cfg.layers[layerIndex].options.sql = filter.sql();
                }
            });
        });
    }
