...
function getLayersLists(layers) {
    return layers.reduce(function(lists, layer) {
        if (layer.lists) {
            Object.keys(layer.lists).forEach(function(listName) {
                lists[listName] = new List(layer.lists[listName], layer.options.sql);
            });
        }
        return lists;
    }, {});
...
MapConfig.prototype.getLists = function() {
    var layersLists = getLayersLists(this.getLayers());
    var configLists = this._cfg.lists || {};

    return Object.keys(configLists).reduce(function(lists, layerName) {
        lists[layerName] = new List(configLists[layerName]);
        return lists;
    }, layersLists);
};
