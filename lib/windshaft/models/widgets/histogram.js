...
var binsQueryTpl = dot.template([
    'bins AS (',
    '  SELECT CASE WHEN total_rows = 0 OR iqr = 0 THEN 1 ELSE GREATEST(4, LEAST(cast(((max_val - min_val) / (2 * iqr * power(total_rows, 1/3))) as int), 10)) END AS bins_number',
...
var histogramQueryTpl = dot.template([
    ...
    '    bins_number,',
    '    nulls_count,',
    '    CASE WHEN min_val = max_val THEN 0 ELSE width_bucket({{=it._column}}, min_val, max_val, bins_number - 1) - 1 END AS bin,',
