...
Histogram.prototype.sql = function(psql, filters, override, callback) {
    if (!callback) {
        callback = override;
        override = {};
    }
    ...
    var columnTypeQuery = format('SELECT pg_typeof({column})::oid FROM ({query}) _cdb_histogram_column_type limit 1', {
    ...
    if (this._columnType === null) {
        psql.query(columnTypeQuery, function(err, result) {
            ...
            self.sql(psql, filters, override, callback);
    ...
    var _query = QueryBuilder.getSql(this.query, filters, _.extend({columnType: this._columnType}, override));
