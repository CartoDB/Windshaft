...
var formulaQueryTpl = dot.template([
    'SELECT',
    '{{=it._operation}}({{=it._column}}) AS {{=it._operation}},',
...
Formula.prototype.sql = function(filters) {
...
Formula.prototype.format = function(result) {
    var formattedResult = {};
    formattedResult[this.operation] = 0;

    if (result.rows.length) {
        formattedResult[this.operation] = result.rows[0][this.operation];
