...
module.exports = function(timeout, mml_store, mapnik_opts) {
    ...
    me.printMap = function(req, callback) {
        var that = this;
        ...
        Step(
            function checkParams() {
              if (this.width < 0 || this.width > MAX_SIZE ||
                  this.height < 0 || this.height > MAX_SIZE) {
                throw new Error('invalid dimensions, valid range [0, ' + MAX_SIZE + ']');
              }
              this(null);
            },
            ...
            function loadMapnik(err, data){
              var that = this;
                ...
                var w = parseFloat(params.width);
                var h = parseFloat(params.height);
                var map = new mapnik.Map (w, h);
                map.fromStringSync(data);
                if (params.bbox) {
                  var bbox = parseBBox(params.bbox);
                  console.log("**********", bbox);
                  map.zoomToBox(bbox);
                } else {
                  map.zoomAll();
