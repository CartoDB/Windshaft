...
module.exports = function(timeout, mml_store, mapnik_opts) {
        ...
        if ( ! cache_entry || cache_entry.cache_buster != cache_buster ) {
            ...
            Step(
              ...
              function makeRenderer (err) {
                ...
                that.makeRenderer(req, cache_entry.setReady.bind(cache_entry));
              }
            );
        }
    ...
    me.makeRenderer = function(req, callback){
        ...
        // if null, encase in array (should never happen, we'd bail out earlier)
        if (_.isNull(params.interactivity)){
            params.interactivity = [ null ];
        }

        params.interactivity = {
            layer: params.layer ? params.layer : params.table,
        ...
        Step(
            ...
            function addInteractivity(err, xml){
              if (err) throw err;
//console.log("XML0: " + xml);
              if ( params.interactivity.layer ) {
                var pat_ilayer = /(<Parameter name="interactivity_layer">)[^\<]*(<\/Parameter>)/;
                xml = xml.replace(pat_ilayer, "$1" + params.interactivity.layer + "$2");
              }
              if ( params.interactivity.fields ) {
                var pat_ifield = /(<Parameter name="interactivity_fields">)[^\<]*(<\/Parameter>)/;
                xml = xml.replace(pat_ifield, "$1" + params.interactivity.fields.join(',') + "$2");
              }
