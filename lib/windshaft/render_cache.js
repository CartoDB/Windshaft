...
var   _                     = require('underscore')
    ...
    , DatasourceFactory     = require('./Datasource.js')
    , util                  = require('util')
...
module.exports = function(timeout, mml_store, map_store, mapnik_opts) {
    ...
    var dsFactory = new DatasourceFactory(mapnik_opts.vector_datasources);

    if ( mapnik_opts.vector_datasources ) {
        tilelive.protocols['windshaft:'] = dsFactory.getHandler();
    }
    ...
    me.mapConfigToMMLBuilderConfig = function(mapConfig) {
        ...
        return { sql: sql, style: style, style_version: style_version, interactivity: interactivity, ttl: 0 }
    };
    ...
    function TileliveAdaptor(renderer, format) {
      ...
      if ( format == 'png' || format == 'mvt' || format == 'mvtz' || format == 'pbf' ) {
    ...
    me.makeRendererMapnik = function(req, callback){
        ...
        Step(
            ...
            function loadMapnik(err, data){
                ...
                var vector_input = mapCfg && mapCfg.obj().hasOwnProperty('datasource');

                if ( params.format == 'mvt' || params.format == 'mvtz' || params.format == 'pbf' ) {
                    uri.protocol = 'bridge:';
                    if ( params.format == 'mvt' ) uri.deflate = false;
                    vector_input = false;
                }
                ...
                if ( vector_input ) {
                    ...
                    dsFactory.loadURI(cfg, dbParams, uri, this);
                } else {
    ...
    me.makeRenderer = function(req, callback){
      var params = req.params;
      if ( params.format.match(/^(png|grid\.json|pbf|mvtz?)$/) ) {
