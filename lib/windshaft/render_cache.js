...
var TorqueRendererFactory = require('./renderers/torque.js');
...
module.exports = function(timeout, mml_store, map_store, mapnik_opts) {
    ...
    var torqueFactory = TorqueRendererFactory;
    ...
    me.mapConfigToMMLBuilderConfig = function(cfg) {
        var sql = [];
        var style = [];
        var interactivity = [];
    ...
    me.makeRendererMapnik = function(req, callback){
        ...
        Step(
            ...
            function initBuilder(err, mapConfig) {
              if (err) throw err;
              if ( params.token ) {
                params = _.defaults(params, that.mapConfigToMMLBuilderConfig(mapConfig.obj()));
                delete params.token;
              }
    ...
    me.makeRendererTorque = function(req, callback){
        ...
        Step(
            ...
            function initRenderer(err, mapConfig) {
              if (err) throw err;
              var dbParams = {}; // TODO: fill this in
              var format = params.format;
              var layer = params.layer;
              torqueFactory.getRenderer(mapConfig, dbParams, format, layer, this);
            },
    ...
    me.makeRenderer = function(req, callback){
      var params = req.params;
      if ( params.format == 'png' || params.format == 'grid.json' ) {
        me.makeRendererMapnik(req, callback);
      }
      else if ( params.format == 'torque' ) {
        me.makeRendererTorque(req, callback);
      }
      else {
        callback(new Error("Unsupported format " + this.format));
      }
    };
