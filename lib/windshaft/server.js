            ...
            function(err, tile, headers) {
                ...
                if (err){
                    ...
                    if(req.params.format) {
                      ...
                      stats_client.increment('windshaft.tiles.' + f + '_error');
                    }
                } else {
                    app.sendWithHeaders(res, tile, 200, headers);
                    if(req.params.format) {
                      var f = req.params.format.replace('.', '_');
                      stats_client.increment('windshaft.tiles.' + f + '_success');
                    }
                    stats_client.increment('windshaft.tiles.success');
                    stats_client.timing('windshaft.tiles.response_time', Date.now() - t0);
                }
            }
        );
    };
