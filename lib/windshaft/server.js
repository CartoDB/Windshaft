...
module.exports = function(opts) {
    ...
    var mmlStore = bootstrapMmlStore(redisPool, opts.grainstore);
    ...
    var rendererFactory = new RendererFactory({
        mapnik: {
            opts: opts.renderer && opts.renderer.mapnik || opts.mapnik,
            mmlStore: mmlStore
        },
    ...
    var staticMapsController = new StaticMapsController(app, render_cache);
...
function bootstrapMmlStore(redisPool, grainstoreOpts) {
    var mmlStore = new grainstore.MMLStore({ pool: redisPool }, grainstoreOpts);

    // Setup localized resources purger
    // TODO: allow ttl to be a configuration option !
    var ONE_DAY_IN_SECONDS = 60*60*24;
    var purger = new LocalizedResourcePurger(mmlStore, ONE_DAY_IN_SECONDS);
    purger.start();
