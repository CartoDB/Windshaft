...
//        base_url_layergroup: base_url_notable + '/layergroup',
...
module.exports = function(opts){
    ...
    var map_store_opts  = { pool_log: false };
    if ( opts.redis ) {
        map_store_opts.redis_host = opts.redis.host;
        map_store_opts.redis_port = opts.redis.port;
        map_store_opts.pool_max   = opts.redis.max;
        map_store_opts.pool_max   = opts.redis.max;
        map_store_opts.pool_idleTimeout = opts.redis.idleTimeoutMillis;
        map_store_opts.pool_reapInterval = opts.redis.reapIntervalMillis;
    }
    ...
    if (!_.isString(opts.base_url_notable) && ! _.isString(opts.base_url_layergroup))
      throw new Error("Must initialise Windshaft with a 'base_url_notable' or 'base_url_layergroup' option");

    opts = _.defaults(opts, {
      base_url_layergroup: opts.base_url_notable + '/layergroup'
    });
...
console.log("getRenderer ERROR: "); console.dir(err);
            ...
            function(err, tile, headers) {
                ...
                if (err) {
                  console.log("getTile ERROR: "); console.dir(err);
                  throw err;
                }
    ...
    ///
    app.getFeatureAttributes = function(req, res) {
        var mapConfig;
        var params;
        Step(
            function (){
...
console.log("req2params ERROR: "); console.dir(err);
            ...
            function getAttributes(err, data) {
                if (err) {
console.log("mapStore.load ERROR: "); console.dir(err);
                  throw err;
                }
                ...
                if ( params.dbpass ) dbParams.pass = params.dbpass;
                ...
                var sql = 'select ' + quoted_att_cols +
                  ' from ( ' + layer.options.sql + ' ) as _windshaft_subquery WHERE ' +
                  pg.quoteIdentifier(fid_col) + ' = ' + params.fid;
            ...
            function formatAttributes(err, data) {
                ...
                if (err) {
                  console.log("getAttributes ERROR: "); console.dir(err);
                  throw err;
                }
                //console.log("DATA: "); console.dir(data);
                if ( data.rows.length != 1 ) {
                  throw new Error(data.rows.length + " features in layer " +
                    params.layer + " of map " + params.token +
    ...
    app.options(app.base_url_layergroup, function(req, res, next) {
    ...
    app.fetchTilesetMetadata = function(req, token, rendererType, layerId, callback) {
        ...
        //console.log("params: "); console.dir(req.params);
    ...
    app.createLayergroup = function(cfg, req, callback) {
        ...
        Step(
                ...
                var next = function(err) {
                  if (! err) finish();
                    ...
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("map_store.del: " + e2);
                      finish(err);
                    });
                  } 
                }
            ...
            function tryFetchTorque(err){
                ...
                var next = function(err) {
                  if ( err ) {
console.log("Error since last attempt to fetch torque tile: " + err);
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting style on torque tile fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
...
console.log("Error since last attempt to fetch torque metadata: " + err);
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting style on torque metadata fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
    ...
    app.post(app.base_url_layergroup, function(req, res){
        var mml_builder;
    ...
    app.get(app.base_url_layergroup, function(req, res){
        var mml_builder;
    ...
    app.get(app.base_url_layergroup + '/:token/:z/:x/:y.:format', function(req, res) {
    ...
    app.get(app.base_url_layergroup + '/:token/:layer/:z/:x/:y.(:format)', function(req, res) {
    ...
    // Gets attributes for a given layer feature 
    app.get(app.base_url_layergroup + '/:token/:layer/attributes/:fid', function(req, res) {
