...
var mapnikTokens_RE = RegExp(/!bbox!|!pixel_width!|!pixel_height!/);
...
module.exports = function(opts){
    ...
    app.findStatusCode = function(err) {
      ...
      if ( err.http_status ) statusCode = err.http_status;
        ...
        else if (errMsg.match(/Postgis Plugin.*ERROR.*column.*does not exist/i)) {
            statusCode = 400;
        }
    ...
    */
    ...
    app.get('/', function(req, res){
        req.profiler.start('windshaft.get_slash');
        app.sendResponse(res, ["JOHN RAMBO"]);
    });

    // version
    app.get('/version', function(req, res){
        req.profiler.start('windshaft.get_version');
    ...
    app.finalizeGetTileOrGrid = function(err, req, res, tile, headers) {
      if (err){
          ...
          if(req.params.format) {
            statsClient.increment('windshaft.tiles.' + req.params.format.replace('.', '_') + '.error');
          }
      } else {
          app.sendWithHeaders(res, tile, 200, headers);
          statsClient.increment('windshaft.tiles.success');
          if(req.params.format) {
            statsClient.increment('windshaft.tiles.' + req.params.format.replace('.', '_') + '.success');
          }
      }
