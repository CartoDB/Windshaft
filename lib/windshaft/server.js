      ...
      if ( req.profiler ) req.profiler.done('beforeTileRender');
            ...
            function(err, tile, headers) {
                ...
                if (err){
                    ...
                    //res.send({ error: err.message }, statusCode);
                    stats_client.increment('windshaft.tiles_error');
                    if(req.params.format) {
                      stats_client.increment('windshaft.tiles_' + req.params.format + '_error');
                    }
                } else {
                    app.sendWithHeaders(res, tile, 200, headers);
                    if(req.params.format) {
                      stats_client.increment('windshaft.tiles_' + req.params.format + '_success');
                    }
                    stats_client.increment('windshaft.tiles_success');
                    stats_client.timing('windshaft.tiles_response_time', Date.now() - t0);
                }
            }
        );
    };
