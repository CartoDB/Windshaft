...
module.exports = function(opts){
    ...
    _.defaults(app, {
        ...
        afterLayergroupCreate: function(req, response, callback) {
            callback(null);
        },
        // Set new map style
    ...
    */
        ...
        Step(
            ...
            function initLayergroup(err, data){
                ...
                for ( var i=0; i<cfg.layers.length; ++i ) {
                  ...
                  if ( lyropt.hasOwnProperty('interactivity') ) {
                    interactivity.layer = 'layer'+i;
                    interactivity.fields = lyropt.interactivity;
                  }
                }

                // interactivity used to be a string as of version 1.0.0
                if ( _.isString(interactivity.fields) ) {
            ...
            function posLayerCreate(err) {
                if (err) throw err;
                app.afterLayergroupCreate(req, response, this);
            },
            function finish(err){
                var statusCode = 200;
                if (err){
    ...
    app.get(app.base_url_notable + '/layergroup/:token/:z/:x/:y.*', function(req, res) {
      req.params.format = req.params['0'];
      app.doCORS(res);
      app.getTileOrGrid(req, res);
    });
