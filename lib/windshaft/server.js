        ...
        Step(
            ...
            function fetchTorqueMetadata(err) {
                ...
                mapConfig.getLayers().forEach(function(lyr, i) {
                    if (mapConfig.layerType(i) === 'torque') {
                        torqueLayers.push(i);
                    }
                });
                ...
                var next = function(err, meta) {
                    if ( err || ( next_layer && ! meta ) ) {
                        ...
                        map_store.del(mapConfig.id(), function(e2) {
                            if (e2) console.error("Deleting MapConfig " + mapConfig.id() + " on torque metadata fetching error: " + e2);
                            finish(err);
                        });
                        return;
                    }
