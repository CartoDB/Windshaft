    ...
    app.post(app.base_url_notable + '/layergroup', function(req, res){
        ...
        Step(
            ...
            function initLayergroup(err, data){
                var next = this;
                if (err) throw err;
                if (_.isUndefined(req.body) || _.isUndefined(req.body.config))
                    throw new Error('must send layergroup configuration in a "config" variable');
                var cfg = JSON.parse(req.body.config);
