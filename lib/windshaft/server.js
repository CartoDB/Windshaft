                ...
                cfg.layers.forEach(function(lyr, i) {
                  ...
                  if (mapConfig.layerType(i) === 'mapnik') {
                    ...
                    if ( lyropt.interactivity ) {
                      ...
                      // [mapnik:0,mapnik:1], so when when trying to get the grid for mapnik:0 it will get the
                      // interactivity for mapnik:1 because we said it was in index=1, and when trying to get the grid
                      // for mapnik:1 it will fail because it will try to reach index=2 in the internal data structure,
                      // which actually has only two elements. That's why we set the grid layer in a zero based index.
