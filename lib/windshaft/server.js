...
console.log("mapStore.load ERROR: "); console.dir(err);
            ...
            function getAttributes(err, pg) {
                if (err) {
console.log("getPGClient ERROR: "); console.dir(err);
                  throw err;
                }
        ...
        Step(
            ...
            function tryFetchGrid(err){
                ...
                var next = function(err) {
                  if ( err ) {
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig on grid fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
...
console.log("Error since last attempt to fetch torque tile: " + err);
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig on torque tile fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
            ...
            function tryFetchAttributes(err){
                ...
                var next = function(err) {
                  if ( err ) {
console.log("Error since last attempt to fetch attributes: " + err);
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig on attributes fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
...
console.log("Error since last attempt to fetch torque metadata: " + err);
                    map_store.del(mapID, function(e2) {
                      if (e2) console.log("Deleting MapConfig on torque metadata fetching error: " + e2);
                      finish(err);
                    });
                    return;
                  }
