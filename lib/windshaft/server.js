...
module.exports = function(opts){
    ...
    _.defaults(app, {
        ...
        //
        setStyle: function(params, style, version, callback) {
            var mml_builder = mml_store.mml_builder(params);
            mml_builder.setStyle(style, callback, version);
        },
        // Delete a map style
        //
        // @param callback function(err, data) where data has currently NO meaning
        ...
        getVersion: function() {
          ...
          version.grainstore = require('../../node_modules/grainstore/package.json').version;
        ...
        Step(
            ...
            function(err, data){
                if (err){
                    ...
                    res.send({style: data.style, version: data.version}, 200);
                }
            }
        );
    });
            ...
            function(err, data){
                ...
                if (_.isUndefined(req.body) || _.isUndefined(req.body.style)) {
                    ...
                    app.beforeStateChange(req, function(err, req) {
                        if ( err ) throw err;
                        app.setStyle(req.params, req.body.style, req.body.version, that);
                    });
                }
            },
            function(err, data) {
