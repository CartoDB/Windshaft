...
module.exports = function(opts){
    ...
    */
    ...
    app.getTileOrGrid = function(req, res){
        ...
        var renderer;
        var t0 = Date.now();
        var t0_getRenderer, t0_getTile, t0_beforeTileRender, t0_afterTileRender, t0_req2params;

        Step(
            function(){
                t0_req2params = Date.now()
                app.req2params(req, this);
            },
            function(err) {
      if ( req.profiler ) req.profiler.done('req2params');
                stats_client.timing('windshaft.tiles.req2params.response_time', Date.now() -  t0_req2params)
                if (err) throw err;
                t0_beforeTileRender = Date.now();
                app.beforeTileRender(req, res, this);
            },
            function(err, data){
                stats_client.timing('windshaft.tiles.beforeTileRender.response_time', Date.now() -  t0_beforeTileRender)
      if ( req.profiler ) req.profiler.done('beforeTileRender');
                ...
                t0_getRenderer = Date.now();
                render_cache.getRenderer(req, this);

            },
            function(err, r, is_cached) {
                stats_client.timing('windshaft.tiles.getRenderer.response_time', Date.now() -  t0_getRenderer)
            ...
            function(err, tile, headers) {
                stats_client.timing('windshaft.tiles.getTile.response_time', Date.now() -  t0_getTile)
                if ( req.profiler ) req.profiler.done('render');
                if (err) throw err;
                t0_afterTileRender = Date.now();
                app.afterTileRender(req, res, tile, headers || {}, this);
            },
            function(err, tile, headers) {
                stats_client.timing('windshaft.tiles.afterTileRender.response_time', Date.now() -  t0_afterTileRender)
                ...
                if (err){
                    ...
                    //res.send({ error: err.message }, statusCode);
                    if ( stats_client ) {
                      // TODO: avoid sending double increment
                      //       when "req.params.format" is set ?
                    ...
                    var process_time = Date.now() - t0;
                    if ( stats_client ) {
                      if(req.params.format) {
                        var f = req.params.format.replace('.', '_');
                        stats_client.increment('windshaft.tiles.' + f + '.success');
                        stats_client.timing('windshaft.tiles.' + f + '.response_time', process_time)
                      }
                      // TODO: avoid sending double increment
                      //       when "req.params.format" is set ?
                      stats_client.increment('windshaft.tiles.success');
                      stats_client.timing('windshaft.tiles.response_time', process_time);
                    }
                }
            }
        );
