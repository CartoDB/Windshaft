...
function MapStore(opts) {
  this._config = _.defaults(opts, {
    ...
    redis_key_mapcfg_expire: 300, // in seconds (7200 is 5 hours; 300 is 5 minutes)
    pool_max: 50,
...
o.load = function(id, callback) {
  ...
  var exp = this._get("redis_key_mapcfg_expire");
  Step(
    function getRecord() {
...
o.save = function(map, callback) {
  ...
  var key = this._get("redis_key_mapcfg_prefix") + id;
  var exp = this._get("redis_key_mapcfg_expire");
  var val = JSON.stringify(map.obj());
  Step(
    function writeRecord() {
      that._redisCmd('SET', [key, val], this);
    },
