...
module.exports = class MapnikFactory {
    ...
    getMetatile (format) {
        let metatile = this._mapnik_opts.metatile;
    ...
    getBufferSize (mapConfig, format, options) {
    ...
    mapConfigToMMLBuilderConfig (mapConfig, queryRewriter, rendererOptions) {
        const options = {
            ids: [],
            sql: [],
            originalSql: [],
        ...
        filteredLayerIndexes.reduce((options, layerIndex) => {
            ...
            const queryOptions = prepareQuery(query, layerOptions.geom_column, layerOptions.geom_type, this._mapnik_opts);
