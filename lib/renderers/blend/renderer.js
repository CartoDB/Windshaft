...
module.exports = class BlendRenderer {
    ...
    async getTile (format, z, x, y) {
        ...
        timer.start('render');
        const data = await Promise.all(this.renderers.map(renderer => {
            return renderer.getTile(format, z, x, y)
                ...
                .catch(err => {
                    if (renderer.renderer.constructor.name === 'HttpRenderer') {
                        if (err.code === 'ETIMEDOUT') {
                            throw err;
                        }
                        return {
                            buffer: EMPTY_IMAGE_BUFFER,
