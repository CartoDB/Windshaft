...
RendererCache.prototype.getStats = function () {
    ...
    return rendererCacheEntries.reduce((accumulatedStats, [cacheKey, cacheEntry]) => {
        const [dbname, token, dbuser, format, layer, scale] = cacheKey.split(':');

        for (let [key, value] of Object.entries({ dbname, token, dbuser, format, layer, scale })) {
            if (key === 'format') {
                value = value === 'grid.json' ? 'grid' : value.replace('.', '-');
            }

            value = value === '' ? `no-${key}` : value;

            const accKey = `rendercache.${key}.${value}`;

            if (accumulatedStats.has(accKey)) {
                accumulatedStats.set(accKey, accumulatedStats.get(accKey) + 1);
            } else {
                accumulatedStats.set(accKey, 1);
            }
        }

        const tokenFormatKey = `rendercache.token-format.${token}.${format === 'grid.json' ? 'grid' : format.replace('.', '-')}`;

        if (accumulatedStats.has(tokenFormatKey)) {
            accumulatedStats.set(tokenFormatKey, accumulatedStats.get(tokenFormatKey) + 1);
        } else {
            accumulatedStats.set(tokenFormatKey, 1);
        }

        const tokenLayerKey = `rendercache.token-layer.${token}.${layer === '' ? 'no-layer' : layer}`;

        if (accumulatedStats.has(tokenLayerKey)) {
            accumulatedStats.set(tokenLayerKey, accumulatedStats.get(tokenLayerKey) + 1);
        } else {
            accumulatedStats.set(tokenLayerKey, 1);
        }
