...
RendererCache.prototype.getStats = function () {
    const rendererCacheEntries = Object.values(this.renderers);
    const stats = new Map();

    stats.set('rendercache.count', rendererCacheEntries.length);

    return rendererCacheEntries.reduce((accumulatedStats, cacheEntry) => {
        const rendererStats = cacheEntry.renderer.getStats();
